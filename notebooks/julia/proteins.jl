...
init_rotation_vec = zeros(3)

if length(ARGS) >= 4
    init_rotation_vec .= parse.(Float64, ARGS[2:4])
else
...
println(string("Ligand rotation vector: ", round.(init_rotation_vec, sigdigits=3)))
...
println(string("Inverse rotation vector: ", round.(inv_rotation_vec, sigdigits=3)))
